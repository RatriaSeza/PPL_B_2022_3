...
if (!isset($_POST['submit'])) {
    $query = " SELECT * FROM khs WHERE nim_mhs='$nim' AND semester_mhs='$sem_mhs'";
    ...
    if (!$result) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            $sks = $row['sks_semester'];
            $ipSem = $row['ip_semester'];
    ...
    // Scan khs
    $scanKHS = upload($nim, $semester);

    //Query update
    if ($scanKHS == '') {
        $query = "UPDATE khs SET sks_semester='$sks', ip_semester='$ipSem' WHERE nim_mhs='$nim' AND semester_mhs='$semester'";
    } else {
        $query = "UPDATE khs SET sks_semester='$sks', ip_semester='$ipSem', berkas_khs='$scanKHS' WHERE nim_mhs='$nim' AND semester_mhs='$semester'";
    }
    ...
    <div class="container flex">
        ...
        <div class="card sky-blue-50 grow" style="padding:50px 70px; margin-top: 10vh; margin-bottom:10vh; margin-right:5vw;">
            <div class="card-header mb-2" style="font-size: 30px">
                <p class="text-gray-700 font-extrabold  ">Edit Data KHS</p>
            </div>
            <div class="card-body">
                <form method="POST" name="fKHS" action="" class="grid dropzone" onsubmit="return validateForm()" enctype="multipart/form-data">
                    <div class="form-group mt-3 mb-7">
                        ...
                        <input name="semester" readonly value="<?= $sem_mhs?>" class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-gray-100 focus:border-gray-500" type="number" placeholder="Masukkan Semester">
                    </div>
                    <div class="form-group mb-7">
                        <label class="block tracking-wide text-gray-700 text-sm font-bold mb-2" for="">
                            SKS Semester
                        </label>
                        <input name="sks" value="<?= $sks; ?>" class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-gray-100 focus:border-gray-500" type="number" placeholder="Masukkan SKS Semester">
                    </div>
                    <div class="form-group mb-7">
                        <label class="block tracking-wide text-gray-700 text-sm font-bold mb-2" for="">
                            IP Semester
                        </label>
                        <input name="ipSem" value="<?= $ipSem; ?>" class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-gray-100 focus:border-gray-500" type="number" step="0.01" placeholder="Masukkan IP Semester (x.xx)">
                    </div>
                    <label class="block tracking-wide text-gray-700 text-sm font-bold mb-2" for="">
                        Upload Scan KHS <span class="text-gray-500 ml-5">*Kosongan jika tidak ada perubahan file</span>
                    </label>
